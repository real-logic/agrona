...
public class ExpandableDirectByteBuffer implements MutableDirectBuffer
{
    ...
    public double parseDoubleAscii(final int index, final int length)
    {
        ...
        final byte first = UNSAFE.getByte(null, address + index);
        if (MINUS_SIGN == first)
        {
            if (1 == length)
            {
            ...
            if (UNSAFE.getByte(null, address + index + 1) == DOUBLE_NEGATIVE_INFINITY_VALUE[1]) // -Infinity
            {
        ...
        final int startCheckAt)
    {
        ...
        {
            if (specialValueBytes[j] != UNSAFE.getByte(null, offset + (index + j)))
            {
                throwParseDoubleError(index, length);
            }
        }
    ...
    private double parsePositiveDoubleAscii(final int index, final int length, final int startIndex)
    {
        ...
        if (i < end && DOT == UNSAFE.getByte(null, offset + i))
        {
            i++;
            while ((end - i) >= 8 && isEightDigitAsciiEncodedNumber(octet = UNSAFE.getLong(null, offset + i)))
            {
        ...
        if (i < end)
        {
            byte b = UNSAFE.getByte(null, offset + i);
            if (LOWER_CASE_E == b || UPPER_CASE_E == b)
            {
                ...
                if (i < end)
                {
                    b = UNSAFE.getByte(null, offset + i);
                    if (MINUS_SIGN == b)
                    {
                    ...
                    while (i < end && isDigit(b = UNSAFE.getByte(null, offset + i)))
                    {
        ...
        if (digitCount > LONG_MAX_DIGITS || LONG_MAX_DIGITS == digitCount && mantissa < 0L)
        {
            ...
            if (digitCount > LONG_MAX_DIGITS || LONG_MAX_DIGITS == digitCount && mantissa < 0L)
            {
                ...
                {
                    mantissa = (mantissa * 10L) + (UNSAFE.getByte(null, offset + i) - 0x30);
                    i++;
                }
