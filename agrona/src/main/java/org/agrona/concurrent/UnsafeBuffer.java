...
public class UnsafeBuffer implements AtomicBuffer
{
    ...
    public int putDoubleAscii(final int index, final double value)
    {
        ...
        if (exp < 0)
        {
            ...
            UNSAFE.putByte(dest, offset, ZERO);
            offset++;
            UNSAFE.putByte(dest, offset, DOT);
            offset++;

            UNSAFE.setMemory(dest, offset, leadingZeros, ZERO);
            offset += leadingZeros;
        ...
        else if (exp >= digitCount - 1)
        {
            ...
            UNSAFE.setMemory(dest, offset, trailingZeros, ZERO);
